...
<p><a href="page_admin_acceuille.php">Retour page d'acceuil</a><p>
        <form method="POST" action="ajoutmodif.php" enctype="multipart/form-data">
            ...
            Description : <textarea name="description" id="description" cols="40" rows="5"><?php $immatriculation = $_POST['immatriculation']; include "connexion.php"; $sql = "SELECT * FROM vehicule WHERE immatriculation='$immatriculation'"; $cible = $connexion->query($sql); $parti = $cible->fetch_assoc(); echo htmlspecialchars($parti['description_vehic']); ?></textarea> <br><br>
            Choisissez une image :<input type="file" name="newImage"><br><br>
